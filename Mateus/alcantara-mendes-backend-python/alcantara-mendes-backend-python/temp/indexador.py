...
diretorio_raiz = Path(r'/home/abenathar/Sistemas/python/alcantara-mendes-chat-python/')
...
def process_documents(folder, service):
    ...
    try:
        documents = SimpleDirectoryReader(str(folder)).load_data()
        index = GPTVectorStoreIndex.from_documents(documents, service_context=service)
        pasta_destino_indices = folder / "indices"
