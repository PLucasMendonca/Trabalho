...
export default function MessageList({ className }: InboxListProps) {
  ...
  useEffect(() => {
    if (importedFile.length >= 1) {
      console.log("Testando", importedFile.length);
      setShowPdf(true);
    }
  }, [importedFile]);
          ...
          {showPdf ? (
            <Worker workerUrl="https://unpkg.com/pdfjs-dist@3.4.120/build/pdf.worker.min.js">
              {importedFile.length >= 1 ? (
                <Viewer fileUrl={importedFile[0].url} plugins={[newPlugin]} />
              ) : (
                importedFile.length == 0 && <span>Visualização do PDF</span>
              )}
            </Worker>
          ) : (
...
export const HistoryList: FC<HistoryProps> = ({ isLoading }) => {
